...
async function cleanTiles() {
  let total = canvas.background.placeables.map(i => i.id); 
  await canvas.scene.deleteEmbeddedDocuments("Tile", total);
}
...
function main() {
            ...
            <details class="custom-sizing-spacing">
                ...
                <fieldset>
                    ...
                    <tbody>
                      <tr>
                        <td>
                          <label for="card-height">Height: <input id="card-height" type="number" min="1" style="width: 50px;" value="350"></label>
                        </td>
                        <td>
                          <label for="card-width">Width: <input id="card-width" type="number" min="1" style="width: 50px;" value="250"></label>
                        </td>
                      </tr>
                    </tbody>
                    </table>
                ...
                <fieldset>
                    <legend>Spacing between Cards (in pixels)</legend>
                      ...
                      <tbody>
                        <tr>
                          <td>
                            <label for="spacing-x">Columns: <input id="spacing-x" type="number" min="0" value="10" step="1" style="width: 50px;"></label>
                          </td>
                          <td>
                            <label for="spacing-y">Rows: <input id="spacing-y" type="number" min="0" value="10" step="1" style="width: 50px;"></label>
                          </td>
                        </tr>
                      </tbody>
                      </table>
    ...
    new Dialog({
        ...
        buttons: {
            ...
            clean: {
                label: `<b style="color:red">Delete All Tiles!!!</b>`,
                callback: async () => {
                  cleanTiles();
                },
            },            
