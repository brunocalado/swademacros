const potionName = 'Potion of Strong Healing';
...
async function main() {
  ...
  let newHealth = actor.data.data.wounds.value - 2;

  //update the actor health
  await actor.update({"data.wounds.value": newHealth});

  sm.styledChatMessage(potionName, '', `${tokenD.actor.name} drank a ${potionName} Healing and cured 2 wound.`);
}