const potionName = 'Potion of Strong Invigoration';
...
async function main() {
  ...
  if(actor.data.data.fatigue.value == 0) {
    sm.styledChatMessage(potionName, '', `You are not fatigued.`);       
    return;
  }
  ...
  let newHealth = actor.data.data.fatigue.value - 2;

  //update the actor health
  await actor.update({"data.fatigue.value": newHealth});

  sm.styledChatMessage(potionName, '', `${tokenD.actor.name} drank a ${potionName} and removed 2 level of fatigue`);
}